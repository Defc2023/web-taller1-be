    ...
    <article className="bg-white border border-gray-200 rounded-xl overflow-hidden max-w-[468px] w-full">
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="relative">
          <button onClick={() => setShowMenu((v) => !v)} className="text-gray-600" aria-label="More options">
            <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
              <path fillRule="evenodd" d="M4.5 12a1.5 1.5 0 113 0 1.5 1.5 0 01-3 0zm6 0a1.5 1.5 0 113 0 1.5 1.5 0 01-3 0zm6 0a1.5 1.5 0 113 0 1.5 1.5 0 01-3 0z" clipRule="evenodd" />
            </svg>
          </button>
          {showMenu && (
            <div className="absolute right-0 top-8 bg-white border border-gray-200 rounded-xl shadow-lg py-1 z-20 w-48">
              <button
                onClick={() => {
                  navigator.clipboard.writeText(window.location.origin + `/api/posts/${post.id}`);
      ...
      <div className="px-4 pt-3 pb-1">
        <div className="flex items-center gap-3 mb-2">
          ...
          <button onClick={() => commentInputRef.current?.focus()} aria-label="Comment">
